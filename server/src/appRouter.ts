...
import Coin, { CoinHistory, CoinHistoryIntervalList, StorageCoin, StorageCoinHistory } from './models/Coin';
import {
	formatSupply,
	formatMarketCap,
	formatVolumeUsd24Hr,
...
const appRouter = trpc.router({
	...
	getCoinHistory: trpc.procedure
		.input((value): getCoinHistoryInput => {
			...
			const isValidId = value?.hasOwnProperty('id')
				&& typeof valueAsType.id === 'string';

			const isValidInterval = value?.hasOwnProperty('interval')
				&& typeof valueAsType.interval === 'string'
				&& valueAsType.interval === valueAsType.interval as CoinHistoryIntervalList;

			const isValidEnd = value?.hasOwnProperty('end')
				&& (
			...
			const isValidStart = value?.hasOwnProperty('start')
				&& (
			...
			if (isValidObject
				...
				&& isValidEnd
				&& isValidStart) {
				const endDate = valueAsType.end === null
					? null
					: new Date(valueAsType.end);
				const startDate = valueAsType.start === null
					? null
