...
class CoinCapController {
    ...
    async getCoinHistory(id: string, interval: CoinHistoryIntervalList) {
        ...
        fetch(`api.coincap.io/v2/assets/${id}/history?interval=${interval}`, requestOptions)
            .then(response => response.text())
            .then(result => console.log(result))
            .catch(error => console.log('error', error));
    }
    ...
    async getCandles(id: string, interval: CandlesIntervalList) {
        ...
        await fetch(`api.coincap.io/v2/candles?exchange=poloniex&interval=${interval}&baseId=ethereum&quoteId=bitcoin\n`, requestOptions)
            .then(response => response.text())
            .then(result => console.log(result))
            .catch(error => console.log('error', error));
    }
